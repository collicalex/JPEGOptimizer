  ...
  private class JPEGFilesListCellRenderer extends DefaultListCellRenderer {
    ...
    private String getEarnStr(JPEGFiles jpgFiles) {
      if (jpgFiles.getState() == JPEGFiles.NOT_YET_OPTIMIZED) {
        ...
        return "<font color='#FF6600'>Optimizing in progress... (" + jpgFiles.getCurrentOptimStep() + "/"
            + jpgFiles.getMaxOptimStep() + ")</font>";
      } else if (jpgFiles.getState() == JPEGFiles.OPTIMIZED_KO) {
        return "<font color='red'>Unable to optimize file (too many visual difference when compressing). Failed in "
            + ReadableUtils.interval(jpgFiles.getElaspedTime()) + "</font>";
      } else if (jpgFiles.getState() == JPEGFiles.OPTIMIZED_OK) {
        if (jpgFiles.getEarnRate() != null) {
          return "<font color='green'>Optimization done. Earn " + ReadableUtils.fileSize(jpgFiles.getEarnSize()) + " ("
              + ReadableUtils.rate(jpgFiles.getEarnRate()) + ") in " + ReadableUtils.interval(jpgFiles.getElaspedTime())
              + ". Jpeg compression quality found is <b>" + jpgFiles.getJpegQualityFound() + "%</b>.</font>";
        }
    ...
    public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
      ...
      l.setText("<html>src : " + getSrcFileStr(jpegFiles) + "<BR>dst : " + getDstFileStr(jpegFiles) + "<BR>"
          + getEarnStr(jpegFiles) + "</html>");
